...
const Restaurant = ({ route, navigation }) => {
  ...
  const editOrder = (action, menuId, price) => {
    ...
    let item = orderList.filter((x) => x.menuId === menuId);
  ...
  const getOrderQty = (menuId) => {
    let orderItem = orderItems.filter((x) => x.menuId === menuId);
  ...
  const renderDots = () => (
    <View style={{ height: 30 }}>
      ...
      >
        {restaurant?.menu.map((item, index) => {
          const opactiy = dotPosition.interpolate({
    ...
    <View>
      ...
      >
        ...
        >
          ...
          <Text style={{ ...FONTS.h3 }}>{getSumOfOrder()}</Text>
        </View>
